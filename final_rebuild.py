...
# 1. Restore app.js from git
os.system('git restore app.js')

with open('app.js', 'r', encoding='utf-8') as f:
    app_js = f.read()

# 2. Fix renderTable safely
...
# 3. Add Safe Wrappers for initDashboard
...
# 4. Safe Wrappers for lucide and calcROI
...
# 5. Extract properly without double escaping
...
for pattern, name in functions:
    ...
    if m:
        ...
        # Fallback to older if it fails (like renderAnalysisChart which takes args)
        if name == 'renderAnalysisChart':
            m2 = re.search(r'function renderAnalysisChart\(.*?\{.*?(?=function calcROI)', orig, flags=re.DOTALL)
            if m2:
                extracted.append(m2.group(0))
            else:
...
print("App.js absolutely perfectly built from scratch.")
