    ...
    // ── IMÁGENES (Unsplash Source — gratis, sin API key) ─────────────────────
    const CAT_IMG_KW = {
    ...
    // ── SKELETON CARDS ───────────────────────────────────────────────────────
    function renderSkeletonCards(containerId, count=8) {
      const container = document.getElementById(containerId);
      if (!container) return;
      container.innerHTML = Array.from({length: count}).map(() => `
        <div class="bg-white/5 border border-white/10 rounded-[2rem] p-4 space-y-4">
    ...
    function init(){
      ...
      renderSkeletonCards('landingProducts', 8);
      // Load products
      loadProducts();
    ...
    // ── FAQ ACCORDION ────────────────────────────────────────────────────────
    const FAQ_ITEMS = [
      { q: '¿Qué incluye el plan gratuito?', a: 'El plan Free te da acceso a los 20 productos con mayor TrendScore, actualizados diariamente. Sin tarjeta de crédito.' },
      { q: '¿Cada cuánto se actualizan los productos?', a: 'La base de datos se regenera cada 6 horas con IA que analiza señales en TikTok, Amazon, Mercado Libre, Instagram y más.' },
      { q: '¿Funciona para vender en Mercado Libre?', a: 'Sí. Filtramos específicamente por plataforma. Podés ver qué productos están trending en Mercado Libre con sus precios estimados para Argentina, Uruguay y Chile.' },
      { q: '¿Puedo cancelar cuando quiero?', a: 'Sí, sin penalidades. Podés cancelar desde tu perfil en cualquier momento y seguir usando el plan hasta fin del período.' },
      { q: '¿El TrendScore qué mide exactamente?', a: 'Combina volumen de búsqueda, tasa de crecimiento en los últimos 7 días, nivel de interacción en redes sociales, ventas históricas y competencia. Un score de 90+ indica producto en plena explosión viral.' },
      { q: '¿Tienen app móvil?', a: 'TrendBase es una Progressive Web App (PWA). Podés instalarla en tu celular desde el navegador — funciona como app nativa sin pasar por el App Store.' },
    ];
    ...
    function initFAQ() {
      ...
      container.innerHTML = FAQ_ITEMS.map((item, i) => `
        <div class="border border-white/10 rounded-2xl overflow-hidden">
          <button onclick="toggleFAQ(${i})" class="w-full flex items-center justify-between px-6 py-5 text-left hover:bg-white/5 transition group">
            <span class="text-sm font-semibold text-white pr-4">${item.q}</span>
            <span id="faq-icon-${i}" class="text-champagne/70 flex-shrink-0 transition-transform duration-300 text-lg font-light">+</span>
          </button>
          <div id="faq-body-${i}" class="hidden px-6 pb-5 text-sm text-white/60 leading-relaxed border-t border-white/5">
    ...
    function toggleMobileMenu() {
      ...
      if (mobileMenuOpen) {
        ...
        // Animate hamburger → X
        document.getElementById('ham1').style.transform = 'rotate(45deg) translate(3px, 3px)';
        document.getElementById('ham2').style.opacity = '0';
        document.getElementById('ham3').style.transform = 'rotate(-45deg) translate(3px, -3px)';
        document.getElementById('ham3').style.width = '1rem';
      } else {
        drawer.classList.add('hidden');
        document.body.style.overflow = '';
        document.getElementById('ham1').style.transform = '';
        document.getElementById('ham2').style.opacity = '';
        document.getElementById('ham3').style.transform = '';
        document.getElementById('ham3').style.width = '';
      }
    }
        ...
        return `
          <div onclick="openProduct(${i})" class="bg-white/5 border border-white/10 rounded-[2rem] overflow-hidden hover-lift shadow-sm cursor-pointer p-4 space-y-4">
            <div class="aspect-video w-full rounded-2xl overflow-hidden bg-black/20 relative">
              <img src="${image}" class="w-full h-full object-cover" loading="lazy" decoding="async" onerror="this.src='https://source.unsplash.com/featured/600x400/?product'">
    ...
    function renderProducts(resetPage=false) {
      ...
      let allFiltered = ALL_PRODUCTS.filter(p => {
        ...
        if(searchQuery && !p.name.toLowerCase().includes(searchQuery) && !p.cat.toLowerCase().includes(searchQuery)) return false;
        return true;
      });
    ...
    function exportCSV() {
      const products = ALL_PRODUCTS.filter(p => {
        ...
        return true;
      }).slice(0, currentPlan().maxProducts);

      if (!products.length) { toast('No hay productos para exportar', 'error'); return; }

      const headers = ['Nombre','Categoría','TrendScore','Cambio 24hs','Margen','Competencia','Precio est.','Plataformas','Regiones'];
      ...
      toast(`${products.length} productos exportados`, 'success');
    }
    ...
    async function subscribe(planName) {
      ...
      try {
        ...
        toast('Error al procesar el pago: ' + e.message, 'error', 5000);
    ...
    function toggleSave(idx) {
      ...
      if(i === -1) {
        saved.push(idx);
        const name = ALL_PRODUCTS[idx]?.name;
        toast(name ? `"${name.slice(0,30)}" guardado` : 'Producto guardado', 'success');
      } else {
        saved.splice(i, 1);
        toast('Producto eliminado de guardados', 'info');
      }
          ...
          ${saved.map(idx => {
            ...
            return `
              <div onclick="openProduct(${idx})" class="bg-white/5 border border-white/10 rounded-[2rem] overflow-hidden hover-lift p-4 space-y-4 cursor-pointer">
                <div class="aspect-video w-full rounded-2xl overflow-hidden bg-black/20 relative">
                  <img src="${p.img || 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?q=80&w=600'}" class="w-full h-full object-cover" loading="lazy" decoding="async">
                  <span class="absolute top-3 left-3 text-[9px] font-mono font-bold uppercase bg-champagne text-obsidian px-2 py-0.5 rounded">${p.cat}</span>
                </div>
                <div class="space-y-2">
                  <h4 class="text-sm font-bold text-white truncate">${p.name}</h4>
    ...
    function generateMarketingCopy() {
      ...
      const user = supabase.auth.getUser ? supabase.auth.getUser() : Promise.resolve({ data: { user: null } });
      (user.then ? user : Promise.resolve(user)).then(({ data: { user: u } }) => {
        const token = u?.access_token || '';
        return fetch('/api/describe', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ product: p, plan: plan.id, token })
        });
    ...
    function _setMktState(state, msg) {
      ...
      if (state === 'loading') {
        ...
        const wrap = document.getElementById('mkt-error');
    ...
    function _renderMkt({ copy, model }, fromCache) {
      ...
      // Add cache badge if from cache
      const badge = document.getElementById('mkt-model-badge');
      if (badge && fromCache) {
        badge.innerHTML += ' <span class="ml-1 text-white/25">(caché)</span>';
      }
    ...
    // Patch openProductModal to reset marketing tab state
    const _origOpenProdModal = typeof openProductModal === 'function' ? openProductModal : null;
