...
def get_section(start_marker, end_marker):
    ...
    if start == -1 or end == -1:
        print(f"Failed to extract {start_marker}")
        return ""
    return html[start:end]

# Extract precisely
nav = html[:html.find('<!-- Hero Section -->')]
hero = get_section('<!-- Hero Section -->', '<!-- Video Demo Section')
video = get_section('<!-- Video Demo Section', '<!-- Trust Element: Integration Banner -->')
...
founder = get_section('<!-- FOUNDER NOTE -->', '<!-- Manifesto Section')
manifesto = get_section('<!-- Manifesto Section', '<!-- Pricing Section')
pricing = get_section('<!-- Pricing Section', '<!-- FAQ Section -->')
...
stats = stats.replace('text-white', 'text-gray-900')
...
founder = founder.replace('bg-white saas-shadow', 'bg-[#141416] border border-white/5 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)]')
