    ...
    function renderAnalysisHistory() {
      ...
      renderAnalysisChart(labelsSlice, historySlice, p.name);

      // --- MOCK DYNAMIC WIDGETS ---
      // Proveedores
      const provCosts = [4.20, 3.50, 6.10, 2.90, 8.40, 5.50];
      let cost1 = provCosts[idx % provCosts.length];
      let cost2 = cost1 * 1.2;
      
      if (businessMode === 'ecommerce') {
        // En ecommerce los precios son más baratos (mayorista)
        cost1 = cost1 * 0.4;
        cost2 = cost1 * 1.1;
        ...
        document.getElementById('provDesc2').innerText = "Personalización + Stock Local";
      } else {
      ...
      // Saturacion
      const satLevels = [24, 65, 89, 12, 45, 78];
      const satIdx = idx % satLevels.length;
      const satVal = satLevels[satIdx];
      const elSatText = document.getElementById('satText');
      if(elSatText) {
        elSatText.innerText = `${satVal}%`;
        elSatText.className = satVal < 40 ? 'text-2xl font-extrabold text-green-400' : (satVal < 70 ? 'text-2xl font-extrabold text-yellow-400' : 'text-2xl font-extrabold text-red-400');
      }
      const elSatStores = document.getElementById('satStores');
      if(elSatStores) {
        elSatStores.innerHTML = `El volumen de búsquedas está subiendo rápido, pero solo hay <strong class="text-white">${Math.floor(satVal * 1.5)} tiendas</strong> escalándolo en LATAM actualmente.`;
      }
    ...
    async function connectStore(platform) {
      ...
      overlay.innerHTML = `
        <div class="bg-obsidian border border-white/10 rounded-[2.5rem] w-full max-w-lg overflow-hidden shadow-2xl">
          ...
          <div class="px-8 py-6 space-y-4">
            ...
            <div id="storeConnectError" class="hidden text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-3"></div>
          </div>
            ...
            <div class="bg-black/30 border border-white/5 rounded-2xl p-4 flex items-center justify-between hover:border-champagne/30 transition cursor-pointer group">
              <div class="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div class="text-[10px] ${profile.store==='shopify'?'text-green-400':'text-white/40'} font-mono">${profile.store==='shopify'?'Conectado':'Desconectado'}</div>
                </div>
              </div>
              <button onclick="connectStore('shopify')" class="text-[10px] font-bold ${profile.store==='shopify'?'bg-red-500/10 text-red-400 hover:bg-red-500/20':'bg-white/5 text-white hover:bg-white/10 group-hover:bg-champagne group-hover:text-black'} px-3 py-1.5 rounded-lg transition">${profile.store==='shopify'?'Desconectar':'Conectar'}</button>
            </div>
            ...
            <div class="bg-black/30 border border-white/5 rounded-2xl p-4 flex items-center justify-between hover:border-champagne/30 transition cursor-pointer group">
              <div class="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div class="text-[10px] ${profile.store==='tiendanube'?'text-green-400':'text-white/40'} font-mono">${profile.store==='tiendanube'?'Conectado':'Desconectado'}</div>
                </div>
              </div>
          ...
          ${cols.map(col => `
            <div class="min-w-[260px] w-[260px] flex-shrink-0 flex flex-col gap-3 snap-start">
              ...
              <div class="flex flex-col gap-3 min-h-[500px]">
                ${col.items.map(item => `
                  <div onclick="openProduct(${item.idx})" class="bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover-lift p-3 space-y-3 cursor-pointer group">
                    ...
                    <div>
                      ...
                      <div class="flex justify-between items-center text-[9px] text-white/50 border-t border-white/5 pt-2">
                        <span>Margen: <b class="text-green-400 font-mono">${item.p.marginStr}</b></span>
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ${SUPPLIERS_BUY.map(s => {
              ...
              return `
                <a href="${s.url + q + (s.aff || '')}" target="_blank" class="bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl p-4 flex flex-col justify-between transition duration-200 hover:-translate-y-1">
                  <div>
